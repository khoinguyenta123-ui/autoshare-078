...
- Recent articles/ideas already published (if any) — used ONLY to avoid repetition, never to inform
  content directly
...
# DIVERSITY CONSTRAINT (read this before everything else below)

Do not repeatedly use the same subtopic, argument, emotional framing, or life lesson as recent
articles. Prefer a genuinely different angle within the main topic.

If a RECENT ARTICLES / RECENT IDEAS list is provided in the context, before generating the new
insight:

- Identify the dominant themes, arguments, and emotional framings already used in that list.
- Reject any candidate idea that substantially repeats them, even if the topic word is the same.
- Choose a different angle, contradiction, concrete situation, or audience experience than what
  the recent list already covers.
- Do not merely replace words while keeping the same underlying message — the CONTRAST itself
  (Step 1-3 below) must be structurally different, not a synonym-swapped version of a recent one.

If no RECENT ARTICLES list is provided, or the topic is broad enough that no real overlap risk
...
# WRITING ENGINE SELECTION

Before output, decide which of three writing engines fits this specific insight best. This choice gets
passed to the Writer, who will follow that engine's method exactly.

**anonymous_narrative** — use when the insight is a general reflective observation about ordinary life,
work, relationships, or habits, without one single small detail that dramatically leverages into a much
larger meaning, and without a harmful/warning-worthy behavior at its center. This is the default choice
when neither of the other two clearly fits better.

**leveraged_insight** — use when your insight genuinely has ONE small, concrete, ordinary detail, reason,
or behavior that can act as a lever into a much larger human/family/social meaning. Only choose this if
you can point to that one small detail explicitly — don't force it.

**direct_warning** — use when the topic involves a harmful habit, a behavior people should recognize and
correct before it becomes serious (technology dependence, neglecting health/relationships, wasted time,
consumer/social pressure, etc). The insight should carry real cautionary weight, not just gentle
reflection.

Pick exactly one. Briefly justify the choice in one sentence.
